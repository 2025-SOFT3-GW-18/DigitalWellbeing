...
- Name Input: Grapheme-safe max 10 (IME/Paste-safe)
- Password: 8-16 half-width alphanumeric only (create & edit)
- Next.js page default export is a function component
- Update: Remove right-click/long-press delete; add explicit Debug Mode delete UI
- Update: Remove placeholders "例: たろう" and "例: Abc12345"
==============================================================
*/
"use client";
...
/* ===============================================
 ★ 名前 10 文字制限（グラフェムベース）
=============================================== */
const sliceGraphemes = (str: string, limit: number) => {
  try {
    // @ts-ignore
    const seg = typeof Intl !== 'undefined' && (Intl as any).Segmenter
      ? new (Intl as any).Segmenter('ja', { granularity: 'grapheme' })
      : null;
    if (seg) {
      const out: string[] = [];
      for (const s of (seg as any).segment(str)) {
        out.push(s.segment);
...
const useGraphemeCappedInput = (initial: string, limit: number) => {
  const [value, setValue] = useState(initial);
  const composingRef = useRef(false);
  const onCompositionStart = () => { composingRef.current = true; };
  const onCompositionEnd = (e: React.CompositionEvent<HTMLInputElement>) => {
    composingRef.current = false;
    setValue(sliceGraphemes(e.currentTarget.value, limit));
  };
  const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (composingRef.current) { setValue(e.target.value); return; }
    setValue(sliceGraphemes(e.target.value, limit));
  };
  const onInput = (e: React.FormEvent<HTMLInputElement>) => {
    const el = e.currentTarget as HTMLInputElement;
    const capped = sliceGraphemes(el.value, limit);
    if (el.value !== capped) { el.value = capped; setValue(capped); }
  };
  return { value, setValue, onCompositionStart, onCompositionEnd, onChange, onInput };
};

/* ===============================================
 ★ パスワード（8～16・半角英数字）ユーティリティ
=============================================== */
const PASSWORD_MAX = 16;
const onlyAlnum = (v: string) => v.replace(/[^A-Za-z0-9]/g, '');
const capPassword = (v: string) => onlyAlnum(v).slice(0, PASSWORD_MAX);
const isValidPassword = (v: string) => /^[A-Za-z0-9]{8,16}$/.test(v);

/* ===============================================
 5. コンポーネント定義（グラフ・モーダル等）
=============================================== */
...
const PersonalizeSection = ({ currentUser, appStats, chartjsConstructor, isChartJsLoaded, onOpenSurvey }: any) => {
  ...
  useEffect(() => { if (step === 'result') { window.scrollTo({ top: 0, behavior: 'smooth' }); } }, [step]);
  ...
  const recommendedApps = resultType ? appStats
    .filter((app: AppStat) => resultType.recommendedAppIds.includes(app.id)
  ...
  return (
    <div className="max-w-3xl mx-auto pt-6 animate-fade-in">
      <div className="bg-white p-8 rounded-xl shadow-lg border-t-4 border-teal-500 mb-8 text-center">
        <p className="text-gray-500 font-bold mb-2">{currentUser.name} さんのタイプは…</p>
  ...
  const KnowledgeLink = ({ icon, title, url, isExternal }: any) => (
    <a href={url} target="_blank" rel="noopener noreferrer" className="block p-4 bg-white rounded-lg border border-purple-100 shadow-sm hover:shadow-md transition hover:bg-purple-50 flex items-center">
      ...
      <div className="flex-1">
        ...
        <p className="text-xs text-indigo-500 font-bold flex items-center">{isExternal ? '公式サイトへ移動' : 'Google検索結果を表示'} <span className="ml-1">{isExternal ? '↗' : '🔍'}</span></p>
      </div>
    </a>
  );
  return (
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-[60]" onClick={onClose}>
      <div className="bg-white w-full max-w-md rounded-xl shadow-2xl p-6 relative animate-fade-in" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700 transition" aria-label="閉じる">
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-50 overflow-y-auto" onClick={() => setIsModalOpen(false)}>
      <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-xl shadow-2xl p-6 md:p-8 relative overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-gray-500 hover:text-gray-800 transition p-2 rounded-full bg-gray-100 hover:bg-gray-200" aria-label="閉じる">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
        ...
        {testResult && style ? (
          <div className={`mt-8 p-6 ${style.bg} border-2 ${style.border} rounded-xl shadow-inner animate-fade-in`}>
            ...
            <div className="flex justify-end space-x-3 mt-6">
              <button onClick={resetTest} className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition shadow-sm">再診断する</button>
              <button onClick={() => setIsModalOpen(false)} className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition shadow-md">閉じる</button>
            </div>
          </div>
        ) : (
          <div className="space-y-6">
              ...
              <div key={index} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex flex-wrap gap-3">
                  {options.map((option) => (
                    <label
                      key={option.score}
                      className={`flex items-center p-3 rounded-xl border-2 cursor-pointer transition duration-150 ease-in-out text-sm font-semibold ${option.class} ${testAnswers[index] === option.score ? 'ring-4 ring-offset-2' : ''}`}
                    ...
                    >
                      <input
                        ...
                        name={`question-${index}`}
                        value={option.score}
                        checked={testAnswers[index] === option.score}
            ...
            <div className="flex justify-end space-x-3 p-4">
              <button
                ...
                className={`px-8 py-3 font-bold rounded-lg transition transform hover:scale-[1.01] shadow-lg ${isAllAnswered ? 'bg-indigo-600 hover:bg-indigo-700 text-white' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
              >
...
/* ---------- ユーザー設定モーダル（※使用箇所より上で宣言） ---------- */
const UserSettingsModal = ({ isOpen, onClose, user, users, onUpdateUser, onDeleteUser }: { isOpen: boolean; onClose: () => void; user: User; users: User[]; onUpdateUser: (id: string, name: string, avatar: string, password: string) => void; onDeleteUser: (userId: string) => void; }) => {
  const MAX_NAME_LENGTH = 10;
  const nameInput = useGraphemeCappedInput(user.name, MAX_NAME_LENGTH);
  ...
  const handleSave = () => {
    ...
    if (!isValidPassword(pw)) return alert('パスワードは8～16文字の半角英数字のみで入力してください');
    const isDuplicate = users.some((u: User) => u.name === name && u.id !== user.id);
    if (isDuplicate) return alert('その名前は既に使用されています。別の名前を入力してください。');
    onUpdateUser(user.id, name, avatar, pw);
    onClose();
  };
  const handleDelete = () => {
  ...
  const onPasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => { setPassword(capPassword(e.target.value)); };
  const onPasswordInput = (e: React.FormEvent<HTMLInputElement>) => {
    const el = e.currentTarget as HTMLInputElement;
    const capped = capPassword(el.value);
    if (el.value !== capped) { el.value = capped; setPassword(capped); }
  };
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 z-[80]" onClick={onClose}>
      <div className="bg-white w-full max-w-sm rounded-xl shadow-2xl p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mb-6">
          ...
          <label className="block text-sm font-bold text-gray-600 mb-2">名前（{MAX_NAME_LENGTH}文字以内）</label>
          <input
            type="text"
          ...
          />
          <label className="block text-sm font-bold text-gray-600 mb-2">パスワード（8～16文字・半角英数字）</label>
          <input
            type="password"
            value={password}
            ...
            className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none mb-1"
          />
        </div>
        <div className="space-y-3">
...
/**
 * 変更点
 * - 右クリック／ロングプレスによる削除を完全廃止
 * - 明示的な「🧪 デバッグモード」トグルを追加
 * - デバッグモードON時のみ各ユーザーカードに「削除（デバッグ）」ボタンを表示
 * - 例: たろう／例: Abc12345 のプレースホルダーを削除
 */
const UserSelectionScreen = ({
  ...
  onCreateUser: (name: string, avatar: string, password: string) => void;
  onDeleteUser: (userId: string) => void;
}) => {
  const [mode, setMode] = useState<'select'|'create'|'login'>('select');
  const [loginUser, setLoginUser] = useState<User | null>(null);
  const [debugMode, setDebugMode] = useState<boolean>(false); // デバッグトグル

  const MAX_NAME_LENGTH = 10;
  const nameInput = useGraphemeCappedInput('', MAX_NAME_LENGTH);
  ...
  const handleCreate = () => {
    ...
    if (!name) return alert('名前を入力してください');
    if (!isValidPassword(pw)) return alert('パスワードは8～16文字の半角英数字のみで入力してください');
    const isDuplicate = users.some((u: User) => u.name === name);
    if (isDuplicate) return alert('その名前は既に使用されています。別の名前を入力してください。');
    onCreateUser(name, inputAvatar, pw);
    setMode('select');
  };
  const handleLogin = () => {
    if (!loginUser) return;
  ...
  const onCreatePasswordChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInputPassword(capPassword(e.target.value));
  };
  const onCreatePasswordInput = (e: React.FormEvent<HTMLInputElement>) => {
    const el = e.currentTarget as HTMLInputElement;
    const capped = capPassword(el.value);
    if (el.value !== capped) { el.value = capped; setInputPassword(capped); }
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white max-w-md w-full p-8 rounded-xl shadow-lg animate-fade-in">
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-2 justify-center mb-4 bg-gray-50 p-3 rounded-lg border border-gray-200 max-h-40 overflow-y-auto">
              {AVATAR_OPTIONS.map((emoji) => (
                <button key={emoji} onClick={() => setInputAvatar(emoji)} className={`w-10 h-10 text-2xl rounded-full transition ${inputAvatar === emoji ? 'bg-indigo-100 border-2 border-indigo-500 scale-110' : 'hover:bg-gray-200'}`}>{emoji}</button>
              ))}
            </div>
            <label className="block text-sm font-bold text-gray-600 mb-2">名前（{MAX_NAME_LENGTH}文字以内）</label>
            <input
          ...
          <div className="flex gap-3">
            <button onClick={() => setMode('select')} className="flex-1 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-lg transition">キャンセル</button>
            <button onClick={handleCreate} className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg transition">作成する</button>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
        <div className="bg-white max-w-sm w-full p-8 rounded-xl shadow-lg animate-fade-in text-center">
          ...
          <h2 className="text-xl font-bold text-gray-800 mb-6">{loginUser.name} さんの<br/>パスワードを入力</h2>
          <input
            type="password"
            value={inputPassword}
            onChange={onCreatePasswordChange}
            ...
            className="w-full p-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none text-2xl font-bold mb-6"
            placeholder="********"
            autoFocus
          />
          <div className="flex gap-3">
            <button onClick={() => setMode('select')} className="flex-1 py-3 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold rounded-lg transition">戻る</button>
            <button onClick={handleLogin} className="flex-1 py-3 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg transition">ログイン</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-indigo-50 to-purple-50 p-4">
        ...
        <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-3 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <div className="text-sm">
              <p className="font-bold text-gray-700">デバッグモード</p>
              <p className="text-gray-500 text-xs">ONにすると「削除（デバッグ）」ボタンが表示されます（関連データを含めて完全削除）</p>
            </div>
          </div>
          <label className="inline-flex items-center cursor-pointer">
            <input type="checkbox" className="sr-only peer" checked={debugMode} onChange={(e) => setDebugMode(e.target.checked)} />
            <div className="w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-red-500 transition relative">
              <span className="absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition peer-checked:translate-x-5"></span>
            </div>
            <span className="ml-2 text-xs font-bold">{debugMode ? 'ON' : 'OFF'}</span>
          </label>
        </div>
        {debugMode && (
          <div className="mt-2 bg-red-50 border border-red-200 text-red-700 rounded-lg p-2 text-xs font-bold">
            🧪 デバッグモード ON：削除（デバッグ）でユーザーの履歴・スコア・タイプ結果など全データが削除されます。
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 max-w-2xl w-full">
        {users.map((user: User) => (
          <div
            key={user.id}
            className="bg-white p-6 rounded-xl shadow-md hover:shadow-xl hover:-translate-y-1 transition flex flex-col items-center border border-gray-100"
          >
            <button
              onClick={() => handleStartLogin(user)}
              className="w-full flex flex-col items-center"
              title="クリックしてログイン"
              aria-label={`ユーザーカード：${user.name}。クリックでログイン`}
            >
            ...
            {debugMode && (
              <button
                onClick={() => {
                  if (confirm(`🧪 デバッグ: 「${user.name}」のアカウントをデータごと完全に削除しますか？\n（履歴・スコア・タイプ結果など、すべての関連データも消えます）`)) {
                    onDeleteUser(user.id);
                  }
                }}
                className="mt-3 w-full text-xs bg-red-50 hover:bg-red-100 text-red-700 px-3 py-2 rounded-lg font-bold border border-red-200 transition"
              >
                削除（デバッグ）
              </button>
            )}
        ...
        <button
          ...
          className="bg-gray-100 border-2 border-dashed border-gray-300 p-6 rounded-xl hover:bg-white hover:border-indigo-300 hover:text-indigo-600 transition flex flex-col items-center justify-center group"
        >
...
/* ===============================================
 メインコンテンツ（ログイン後）
=============================================== */
const MainContent = ({ currentUser, users, onLogout, onUpdateUser, onDeleteUser, chartjsConstructor, isChartJsLoaded }: { currentUser: User; users: User[]; onLogout: () => void; onUpdateUser: (id: string, name: string, avatar: string, password: string) => void; onDeleteUser: (userId: string) => void; chartjsConstructor: ChartConstructor; isChartJsLoaded: boolean; }) => {
  ...
  // ★Fix: リロード時にブラウザのスクロール位置復元を無効化し、強制的にトップへ移動
    useEffect(() => {
  ...
  useEffect(() => {
    ...
    setActiveTab(loadFromLocalStorage(KEY_ACTIVE_TAB, 'diagnosis', currentUser.id));
  ...
  useEffect(() => { if (isDataLoaded) saveToLocalStorage(KEY_ACTIVE_TAB, activeTab, currentUser.id); }, [activeTab, currentUser.id, isDataLoaded]);
    ...
    setAppStats((prevStats: AppStat[]) =>
      prevStats.map((app: AppStat) => {
        ...
        return {
          ...
          ratings: {
            effectiveness: updateRating(app.ratings.effectiveness, userRatings.effectiveness),
            fun: updateRating(app.ratings.fun, userRatings.fun),
            ease: updateRating(app.ratings.ease, userRatings.ease),
            continuity: updateRating(app.ratings.continuity, userRatings.continuity),
            design: updateRating(app.ratings.design, userRatings.design),
          }
        };
      })
    );
  ...
  const handleUserDelete = (userId: string) => { onDeleteUser(userId); onLogout(); };
            ...
            <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
              ...
              {displayHistory.length === 0 ? (
                <div className="text-center py-8 text-gray-400">
                  <p className="text-sm">まだ履歴がありません。<br/>テストを受けるとここに記録されます。</p>
                </div>
              ) : (
                <div className="space-y-3">
...
const DigitalWellbeingApp: React.FC = () => {
  ...
  // ユーザー読み込み & 自動ログイン（旧 pin → password 移行）
  useEffect(() => {
    const initApp = () => {
      ...
      if (lastUserId && migratedUsers.length > 0) {
        const targetUser = migratedUsers.find((u: User) => u.id === lastUserId);
  ...
  const handleCreateUser = (name: string, avatar: string, password: string) => {
    const newUser: User = { id: Date.now().toString(), name, avatar, password };
  ...
  const handleUpdateUser = (id: string, name: string, avatar: string, password: string) => {
    const updatedUsers = users.map((u: User) =>
      u.id === id ? { ...u, name, avatar, password } : u
    );
    setUsers(updatedUsers);
    if (currentUser && currentUser.id === id) {
      setCurrentUser({ ...currentUser, name, avatar, password });
    }
  };
  ...
  if (!currentUser) {
    return <UserSelectionScreen users={users} onSelectUser={handleLoginUser} onCreateUser={handleCreateUser} onDeleteUser={handleDeleteUser} />;
  }
  return (
    <MainContent
